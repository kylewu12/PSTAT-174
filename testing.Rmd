...
```{r}
estim_prod <- auto_production %>% filter(DATE < "2020-01-01")

fit_estim_prod <- auto.arima(log(estim_prod$DAUPNSA))
checkresiduals(fit_estim_prod)
forecast_estim <- forecast(fit_estim_prod, h = 20)

fit_basic1 <- auto.arima(log(auto_production$DAUPNSA))
checkresiduals(fit_basic1)
forecast_1 <- forecast(fit_basic1, h = 12)


plot(ts(log(auto_production$DAUPNSA))) + ts(forecast_estim)
```
...
EndOfSample_prod <- seq(2023, 2024, 1/12)
...
# estimate Gas Model
  prod_total_model_auto <- dynlm::dynlm(diff_total_log ~ L(diff_total_log, 1:12) + L(auto_prod, 0:10) + season(diff_total_log), start = c(1993,1), data = total_prod_analysis)
  ...
  total_prod_analysis <- ts(c(total_prod_analysis[, "diff_total_log"], forecasts_prod))
}
