...
chip shortage that occurred as a result of COVID-19 shutdowns led to supply chain issues that in many ways created
...
world decreasing vehicle
production, and partially led to the rapid rise in vehicle prices and sales patterns that broke from previous market trends.
...
answer even more questions regarding the American auto market. If
we take into account other economic factors, such as bank prime rate loans or
...
model to determine how each factor relating to the automotive
industry may affect total new vehicle sales. Besides
...
vehicle purchases in the United States and whether it is likely that vehicle
...
for the expenditure that comes with the purchase of a new car or to understand what economic factors may affect how they will be buying their new vehicles.
...
consumers. The information we find about other factors such as domestic auto production could then also allow us to understand the types of actions the government could take in case of an economic downturn to try to support the auto market.
...
would indicate that the American economy has been healthy and following a
positive trend, whereas decreased vehicle purchases may indicate that
the economy had been following a general undesirable trajectory.

Throughout this case study, we will aim to find the relationship between total new vehicles sold and its relation with unemployment rates, bank prime rate loans, domestic auto production, and gasoline CPI. We will do this through the use of time series linear models which we will then perform forecast analysis on to determine how well they fit the actual data.

## Analysis of Empirical Properties

In all cases, the data I selected came from the Federal Reserve Economic
Data (FRED) database and all variables selected were recorded on a monthly
basis and input into a format that was very neat and simple to use for time
series analysis. As mentioned in the introductionm the variables I have chosen are the Total number of new
...
market, and the other variables will be used as the exogenous predictors.
...
considering that bank prime rate loans are associated with larger
purchases such as for cars or houses.

We will now analyze the variables individually before talking about all
the factors as they may relate to car sales.
...
# Data breakdown for total cars sold

average_total <- mean(total_vehicles$TOTALNSA) 
...
in a easily usable database. The average number of new cars sold over the timer period covered by the data is 1.26 million cars a month. From the subseries plot, we see that the
...
whether new vehicle sales has followed a general trend.
...
average_unemployment <- mean(unemployment$UNRATE)
...
The data for unemployment rate goes back to January 1948, is recorded
on a monthly basis, and is collected by the U.S. Bureau of Labor
Statistics and with no missingess present in this time series. The average unemployment over the period covered by the data has been 5.73%. From
our data, we see general cyclical fluctuations with periods with varying
periods of higher vs lower unemployment. There does not appear to have
been a consistent trend of unemployment over the life cycle of the data.
...
large spike in unemployment following the beginning stages of the
...
average_production <- mean(auto_production$DAUPNSA)
...
U.S. Bureau of Economic Analysis with no missingess present in the data. Over the period of time covered by the data, monthly domestic auto production has averaged about 345 thousand vehicles a month.
...
highlighted a long-term trend that would have occurred regardless. 



```{r, fig.cap="Bank Prime Rate Loan."}
average_bank_rate <- mean(bank_rate$MPRIME)
...
The federal funds rate is often set based on other determinants of the market so we
...
we discussed earlier, we will be able to create a regression model that
...
that would have likely occurred regardless of the pandemic. Over the period of time covered by the data, the average bank prime rate loan has been 6.44%.
...
number of vehicles sold and different economic variables. The selected model type we will use
...
order to construct an ARDL model, we must first examine the data and
...
simpler time series. Adjusting the data will allow us to stabilize the variance of the time series, which will allow us to better model the
data, and lead to better forecasts. For the variables chosen, the
only ones that may need transformations due to variations that change with time
...
change with time, we will apply logarithmic transformations in order to keep the data easily interpretable. 
...
constants and $\epsilon_t$ is a random error term with mean zero. 
...
# Decomposition of Total New Vehicle Sales
...
create stationary time series, which will allow us to model
...
series is the method of differencing, where we compute the difference
...
null and alternative hypothesis respectively. We can now use the KPSS test to determine if our data will require differencing.
...
$$H_A: \text{Series is stationary or series has no unit root.}$$ If our
...
mean that it would be stationary and does not have a time-dependent
structure.
...
we see that all our variables have p-values less than 0.05, meaning
...
time series are non-stationary. To be safe, data identified as
...
to be any defined pattern that this data follows, so we see that differencing the data once allows the time series to become stationary.
...
the variable is regressed on itself. A model that utilizes
...
values of the independent exogenous variable. A distributed lag model of order r
DL(r) can be written as
$$Y_t = \alpha + \sum_{i=1}^rX_{t-i}+\epsilon_t$$
...
variables and the resulting number of vehicles sold.
...
seasonality so in order to account for that in our model, we will add a seasonal term. After taking seasonality into account, we can use
an ANVOA test to determine if the seasonal variable results in a
significantly different model that improves the models predictive capabilities.

```{r}
# Evaluating base AR model and model with seasonal term included

total_log <- ts(log(total_vehicles$TOTALNSA), start = c(1976,01), frequency = 12)

diff_total_log <- diff(total_log)
model_base <- dynlm::dynlm(diff_total_log ~ 1)
model_with_season <- dynlm(diff_total_log ~ season(diff_total_log), data = total_log)
ar_model <- dynlm::dynlm(diff_total_log ~ L(diff_total_log, 1:12) + season(diff_total_log), data = diff_total_log)
...
simultaneously to search for the best possible model. The information
...
remaining residuals having no significant autocorrelation present. This can be done
both with an inspection of residual plots and with the implementation of
...
residuals are evenly distributed. We would also want to make sure that the scale-location plot has a flat
line, indicating constant variance, and that the points on the QQ-plot
lie close to the line. The Residuals vs. Leverage plot would help us identify
...
It is important to note that for a variable to Granger-cause another variable means that it is helpful in forecasting the other variable [@granger].


### Total Vehicles Sold and Domestic Auto Production:

```{r, include = FALSE}
# Selecting best ARDL for domestic auto production as exogenous variable
...
```{r, include = FALSE}
# Granger-causality test for domestic auto production ARDL model
...
```{r}
# Creating data table for unemployment rate and total new vehicles sold
...
```{r, include = FALSE}
# Best ARDL model selection for unemployment rate as exogenous variable
...
```{r, include = FALSE}
# Granger-causality test for unemployment ARDL model
...
```{r}
# Table to show different unemployment ARDL model evaluations
...
```{r}
# Putting data together for Gas CPI and total vehicle analysis
...
```{r, include = FALSE}
# ARDL best model selection for Gasoline CPI as exogenous variable
...
```{r}
# Table to show different models attempted for unemployment as exogenous variable
...
```{r}
# Putting data together for bank prime rate loan and total new vehicles sold
...
```{r, include = FALSE}
# Model evaluation for bank prime rate loan as exogenous variable
...
```{r}
# Table showing different models attempted for bank prime rate loan as exogenous variable
...
test we obtained a p-value of $0.4755 > 0.05$ so we cannot reject the
...
provided by the models. In order to do this, we will utilize the method
of pseudo out-of-sample forecasting (POOSF) which follows the following steps
...
POOSFCE <- c(window(diff_total_log, start = c(2021, 1), end = c(2023, 1))) - forecasts
...
# plot the total new vehicle sales percent change time series
plot(window(diff_total_log, c(2021, 1), c(2023, 1)),
     col = "steelblue",
     lwd = 2,
     ylab = "Percent",
     main = "POOSF Percent Change with Unemployment") +
...
plot(window(diff_total_log, c(2021, 1), c(2023, 1)),
     ...
     lwd = 2,
     ylab = "Percent",
     main = "POOSF Percent Change with Gas CPI") +
...
plot(window(diff_total_log, c(2021, 1), c(2023, 1)),
     ...
     lwd = 2,
     ylab = "Percent",
     main = "POOSF Percent Change with Domestic Auto Production") +
...
Furthermore, as a more holistic study of the economy, since vehicles are often the second most expensive item a family may purchase, we could try to analyze the relationship between vehicles sold and house purchases and see if vehicle sales may have any relation to the housing market.

Ultimately, this case study allowed us to better understand the factors driving Americans to purchase vehicles and its relation to other economic forces.
...
## Unemployment ARDL Summary
```{r}
summary(unemp_total_modelvar_bic)
```


### Gasoline CPI ARDL Summary
```{r}
summary(gas_total_model_autobic)
```


### Domestic Auto Production ARDL Summary
```{r}
summary(prod_total_model_auto)
```

### Bank Prime Rate Loan ARDL Summary
```{r}
summary(rate_total_model_autobic)
```
