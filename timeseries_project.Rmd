...
for(i in 1:length(EndOfSample)) {
  ...
  unemp_total_modelvar_bic <-  dynlm::dynlm(diff_total_log ~ L(diff_total_log, 1:12) + L(unemp, 0:1), start = c(1976,1), end = EndOfSample[i], data = total_unemp_analysis)
  ...
  # compute forecast
  forecasts[i] <- coef(unemp_total_modelvar_bic) %*% c(1, s[1, 1], s[2, 1], s[3,1], s[4,1], s[5,1], s[6,1], s[7,1], s[8,1], s[9,1], s[10,1], s[11,1], s[12, 1], s[12, 2], s[13,2])
}
...
legend("bottomleft", 
       ...
       legend = c("Actual Total New Vehciles Sold",
         "Forecasted Total New Vehicles Sold",
         "Pseudo forecast Error"))

SER[1]
sd(POOSFCE)
Metrics::rmse(window(diff_total_log, c(2020, 1), c(2023, 1)), PSOSSFc)
```
...
# estimate Gas Model
  gas_total_model_autobic <- dynlm::dynlm(diff_total_log ~ L(diff_total_log, 1:12) + L(gas_cpi, 0:12), start = c(1976,1), end = EndOfSample_gas[i], data = total_gas_analysis)
  ...
  # compute forecast
  forecasts_gas[i] <- coef( gas_total_model_autobic) %*% c(1, s[1, 1], s[2, 1], s[3,1], s[4,1], s[5,1], s[6,1], s[7,1], s[8,1], s[9,1], s[10,1], s[11,1], s[12, 1], s[1, 2], s[2, 2], s[3,2], s[4,2], s[5,2], s[6,2], s[7,2], s[8,2], s[9,2], s[10,2], s[11,2], s[12, 2], s[13,2])
}
# compute psuedo-out-of-sample forecast errors
...
SER_gas[1]
sd(POOSFCE_gas)
Metrics::rmse(window(diff_total_log, c(2020, 1), c(2023, 1)), PSOSSFc_gas)
```
...
# estimate Gas Model
  prod_total_model_auto <- dynlm::dynlm(diff_total_log ~ L(diff_total_log, 1:12) + L(auto_prod, 0:10), start = c(1993,1), end = EndOfSample_prod[i], data = total_prod_analysis)
  ...
  # compute forecast
  forecasts_prod[i] <- coef(prod_total_model_auto) %*% c(1, s[1, 1], s[2, 1], s[3,1], s[4,1], s[5,1], s[6,1], s[7,1], s[8,1], s[9,1], s[10,1], s[11,1], s[12, 1], s[3,2], s[4,2], s[5,2], s[6,2], s[7,2], s[8,2], s[9,2], s[10,2], s[11,2], s[12, 2], s[13,2])
}
# compute psuedo-out-of-sample forecast errors
...
SER_prod[1]
sd(POOSFCE_prod)
Metrics::rmse(window(diff_total_log, c(2020, 1), c(2023, 1)), PSOSSFc_prod)
```
