...
test_stat1 <- c(1.3497, 0.0181, 0.0044, 0.0527, 8.2357)
significant_p <- c(0.01, 0.1, 0.1, 0.1, 0.01)
...
```{r}
class <- c("Total Vehicles", "Domestic Production", "Bank Prime Rate", "Uemployment", "Gasoline CPI")
test_stat1 <- c(-2.74, -3.8473, -2.8386, -3.8808, -2.5347)
...
we see that total vehicles and gasoline CPI have p-values less than
...
exception of our total vehicles and unemployment time series, all our
other time series are non-staionary. For the sake of trying to make our
data more easily interpretable, I will difference all the time series
anf then once again conduct the KPSS and ADF tests on them to check for
stationarity.
...
```{r}
plot(ts(difference(log(fuel_prices$CUSR0000SETB01)), start = c(1982,02), frequency = 12), ylab = "Gas CPI")
```
...
ar_model <- dynlm(diff_total_log ~ L(diff_total_log, 1:12) + season(diff_total_log), data = total_prod_analysis)

anova(ar_model, prod_total_model_auto)
```
